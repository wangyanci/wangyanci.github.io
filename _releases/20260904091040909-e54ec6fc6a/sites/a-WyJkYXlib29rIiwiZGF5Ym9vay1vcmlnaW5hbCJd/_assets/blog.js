(function(){
  var contract={"beforeEvent":"daybook:before-swap","afterEvent":"daybook:page-load","mountRootSelector":"#daybook-page","ignoreInitialAfterMount":true};
  function copyDetail(event){var source=event&&event.detail&&typeof event.detail==='object'?event.detail:{};var detail={};Object.keys(source).forEach(function(key){detail[key]=source[key];});return detail;}
  document.addEventListener(contract.beforeEvent,function(event){document.dispatchEvent(new CustomEvent('html-server:before-page-unmount',{detail:copyDetail(event)}));});
  document.addEventListener(contract.afterEvent,function(event){var source=event&&event.detail;if(contract.ignoreInitialAfterMount&&source&&source.navigationType==='initial')return;var main=document.querySelector(contract.mountRootSelector);if(!main)return;var detail=copyDetail(event);detail.main=main;document.dispatchEvent(new CustomEvent('html-server:after-page-mount',{detail:detail}));});
})();
(function(){
  function q(s){return document.querySelector(s)}function qa(s){return Array.prototype.slice.call(document.querySelectorAll(s))}
  var settingDefaults={useSystemCursor:false,disableBackgroundPlayback:true,disableComments:false,reducedMotion:false};
  function loadSettings(){try{return Object.assign({},settingDefaults,JSON.parse(localStorage.getItem('daybook:user-settings')||'{}'));}catch(error){return Object.assign({},settingDefaults);}}
  function syncSettings(value){var html=document.documentElement;html.toggleAttribute('data-use-system-cursor',!!value.useSystemCursor);html.toggleAttribute('data-disable-background-playback',!!value.disableBackgroundPlayback);html.toggleAttribute('data-comments-disabled',!!value.disableComments);html.toggleAttribute('data-reduced-motion',!!value.reducedMotion);qa('[data-daybook-setting]').forEach(function(input){input.checked=!!value[input.getAttribute('data-daybook-setting')];});}
  var userSettings=loadSettings();syncSettings(userSettings);
  function drawer(open){document.body.classList.toggle('is-daybook-drawer-open',open);var panel=q('[data-daybook-drawer]'),button=q('[data-daybook-drawer-open]');if(panel)panel.setAttribute('aria-hidden',String(!open));if(button)button.setAttribute('aria-expanded',String(open));}
  function mobileTags(open){document.body.classList.toggle('is-daybook-tags-overlay-open',open);var panel=q('[data-daybook-mobile-tags-overlay]'),button=q('[data-daybook-mobile-tags]');if(panel)panel.setAttribute('aria-hidden',String(!open));if(button)button.setAttribute('aria-expanded',String(open));if(open)drawer(false);}
  function notesTool(name){var tools=q('[data-daybook-notes-tools]');if(!tools)return;var open=!tools.classList.contains('is-'+name+'-open');tools.classList.toggle('has-open-panel',open);tools.classList.toggle('is-search-open',open&&name==='search');tools.classList.toggle('is-tags-open',open&&name==='tags');qa('[data-daybook-notes-tool]').forEach(function(button){button.setAttribute('aria-expanded',String(open&&button.getAttribute('data-daybook-notes-tool')===name));});qa('.html-server-daybook-tool-panel').forEach(function(panel){panel.setAttribute('aria-hidden',String(!(open&&panel.classList.contains('is-'+name))));});if(open&&name==='search'){var input=q('[data-daybook-notes-search]');if(input)setTimeout(function(){input.focus();},220);}}
  document.addEventListener('click',function(event){var target=event.target.closest&&event.target.closest('[data-daybook-drawer-open],[data-daybook-drawer-close],[data-daybook-mobile-tags],[data-daybook-mobile-tags-close],[data-daybook-settings-open],[data-daybook-settings-close],[data-daybook-motion],[data-daybook-notes-tool]');if(!target)return;
    if(target.hasAttribute('data-daybook-drawer-open'))drawer(true);else if(target.hasAttribute('data-daybook-drawer-close'))drawer(false);else if(target.hasAttribute('data-daybook-mobile-tags'))mobileTags(true);else if(target.hasAttribute('data-daybook-mobile-tags-close'))mobileTags(false);else if(target.hasAttribute('data-daybook-settings-open')){event.preventDefault();var settings=q('[data-daybook-settings]');if(settings){settings.removeAttribute('inert');settings.setAttribute('aria-hidden','false');settings.classList.add('is-open');document.body.classList.add('is-daybook-settings-open');document.body.style.overflow='hidden';}}
    else if(target.hasAttribute('data-daybook-settings-close')){var settings=q('[data-daybook-settings]');if(settings){settings.setAttribute('inert','');settings.setAttribute('aria-hidden','true');settings.classList.remove('is-open');document.body.classList.remove('is-daybook-settings-open');document.body.style.overflow='';}}
    else if(target.hasAttribute('data-daybook-motion')){var reduced=document.body.getAttribute('data-daybook-reduced-motion')!=='true';document.body.setAttribute('data-daybook-reduced-motion',String(reduced));target.setAttribute('aria-pressed',String(reduced));}
    else if(target.hasAttribute('data-daybook-notes-tool'))notesTool(target.getAttribute('data-daybook-notes-tool'));
  });
  document.addEventListener('input',function(event){var input=event.target.closest&&event.target.closest('[data-daybook-notes-search]');if(!input)return;var term=(input.value||'').trim().toLocaleLowerCase();qa('[data-note-card]').forEach(function(card){var value=[card.getAttribute('data-search-title'),card.getAttribute('data-search-summary'),card.getAttribute('data-tags')].join(' ').toLocaleLowerCase();card.hidden=!!term&&!value.includes(term);});qa('.html-server-daybook-month').forEach(function(month){month.hidden=!month.querySelector('[data-note-card]:not([hidden])');});var pinned=q('.html-server-daybook-pinned'),divider=q('.html-server-daybook-notes-divider');if(pinned){pinned.hidden=!pinned.querySelector('[data-note-card]:not([hidden])');if(divider)divider.hidden=pinned.hidden;}var empty=q('.html-server-daybook-filter-empty');if(empty)empty.hidden=!!q('[data-note-card]:not([hidden])');});
  document.addEventListener('keydown',function(event){if(event.key==='Escape'){drawer(false);mobileTags(false);var settings=q('[data-daybook-settings]');if(settings){settings.setAttribute('inert','');settings.setAttribute('aria-hidden','true');settings.classList.remove('is-open');document.body.classList.remove('is-daybook-settings-open');document.body.style.overflow='';}}});
  function stopBackgroundAudio(){qa('audio').forEach(function(audio){if(typeof audio.pause==='function')audio.pause();});}
  document.addEventListener('change',function(event){var input=event.target.closest&&event.target.closest('[data-daybook-setting]');if(!input)return;userSettings[input.getAttribute('data-daybook-setting')]=!!input.checked;try{localStorage.setItem('daybook:user-settings',JSON.stringify(userSettings));}catch(error){}syncSettings(userSettings);if(input.getAttribute('data-daybook-setting')==='disableBackgroundPlayback'&&input.checked)stopBackgroundAudio();document.dispatchEvent(new CustomEvent('daybook:settings-change',{detail:userSettings}));});
  document.addEventListener('html-server:before-page-unmount',function(){if(userSettings.disableBackgroundPlayback)stopBackgroundAudio();});
  var archiveRevealObserver=null,archiveRevealReset=null,archiveRevealIndex=0,archiveSeenRows=new Set();
  function animationTimeMs(value){return String(value||'0s').split(',').reduce(function(max,item){var parsed=parseFloat(item)||0;return Math.max(max,/ms\s*$/.test(item)?parsed:parsed*1000);},0);}
  function triggerArchiveReveal(row,index,mode){var id=row.getAttribute('data-archive-row-id');if(!id||row.classList.contains('is-intro-revealing')||row.classList.contains('reveal-trigger')||row.classList.contains('is-seen'))return;archiveSeenRows.add(id);row.classList.remove('is-pending-reveal');if(archiveRevealObserver)archiveRevealObserver.unobserve(row);if(document.documentElement.hasAttribute('data-reduced-motion')){row.classList.add('is-seen');return;}var className=mode==='intro'?'is-intro-revealing':'reveal-trigger';row.classList.add(className);row.style.setProperty('--stagger-index',String(index));var finish=function(){row.classList.remove(className);row.classList.add('is-seen');};var item=row.querySelector('.archive-item'),fallback=1000;if(item){item.addEventListener('animationend',finish,{once:true});var style=getComputedStyle(item);fallback=animationTimeMs(style.animationDuration)+animationTimeMs(style.animationDelay)+80;}setTimeout(finish,fallback);}
  function mountArchiveReveal(root){if(archiveRevealObserver){archiveRevealObserver.disconnect();archiveRevealObserver=null;}if(archiveRevealReset!==null){clearTimeout(archiveRevealReset);archiveRevealReset=null;}archiveRevealIndex=0;var rows=Array.prototype.slice.call(root.querySelectorAll('.archive-item-row'));if(!rows.length)return;rows.forEach(function(row){var id=row.getAttribute('data-archive-row-id');row.classList.remove('is-intro-revealing','reveal-trigger');if(id&&archiveSeenRows.has(id)){row.classList.remove('is-pending-reveal');row.classList.add('is-seen');}else{row.classList.remove('is-seen');row.classList.add('is-pending-reveal');}});requestAnimationFrame(function(){var introIndex=0,vh=window.innerHeight;rows.forEach(function(row){var rect=row.getBoundingClientRect(),id=row.getAttribute('data-archive-row-id');if(id&&!archiveSeenRows.has(id)&&rect.bottom>0&&rect.top<vh){triggerArchiveReveal(row,Math.min(introIndex,15),'intro');introIndex++;}});archiveRevealObserver=new IntersectionObserver(function(entries){var revealed=false;entries.forEach(function(entry){var row=entry.target,id=row.getAttribute('data-archive-row-id');if(entry.isIntersecting&&id&&!archiveSeenRows.has(id)){triggerArchiveReveal(row,Math.min(archiveRevealIndex,10),'scroll');archiveRevealIndex++;revealed=true;}});if(revealed){if(archiveRevealReset!==null)clearTimeout(archiveRevealReset);archiveRevealReset=setTimeout(function(){archiveRevealIndex=0;},300);}},{rootMargin:'-40px 0px'});rows.forEach(function(row){var id=row.getAttribute('data-archive-row-id');if(id&&!archiveSeenRows.has(id))archiveRevealObserver.observe(row);});});}
  function animateArchiveStat(node){if(node.classList.contains('anim-done'))return;var targetAttr=node.getAttribute('data-target');if(targetAttr===null||targetAttr==='')return;var target=parseInt(targetAttr,10);if(Number.isNaN(target))return;node.classList.add('anim-done');var isK=node.getAttribute('data-format')==='k',displayTarget=isK?target/1000:target;var display=function(value,complete){node.textContent=isK?value.toFixed(1):(complete?displayTarget.toLocaleString():Math.floor(value).toLocaleString());};if(document.documentElement.hasAttribute('data-reduced-motion')){display(displayTarget,true);return;}var start=performance.now(),duration=1200;var tick=function(now){var progress=Math.min(1,(now-start)/duration);progress=progress===1?1:1-Math.pow(2,-10*progress);display(displayTarget*progress,progress===1);if(progress<1)requestAnimationFrame(tick);};requestAnimationFrame(tick);}
  function mountArchiveStats(root){var scope=root&&root.querySelectorAll?root:document;Array.prototype.slice.call(scope.querySelectorAll('.archive-stat-num[data-daybook-stat]:not(.anim-done),[data-site-visitors-anim]:not(.anim-done),[data-site-views-anim]:not(.anim-done),[data-site-viewers][data-target]:not(.anim-done)')).forEach(animateArchiveStat);}
  var archiveWordTotals={};
  function currentArchiveRoot(lang){var roots=qa('[data-daybook-archive-language="'+lang+'"]');return roots[roots.length-1];}
  function applyArchiveWordTotal(lang,total,root){archiveWordTotals[lang]=total;var target=root&&root.isConnected?root:currentArchiveRoot(lang);if(!target)return;var pending=target.querySelector('[data-daybook-word-stat-pending]');if(!pending)return;var node=pending.querySelector('.archive-stat-num');if(!node)return;pending.removeAttribute('data-daybook-word-stat-pending');node.setAttribute('data-target',String(total));node.setAttribute('data-daybook-stat','');node.classList.remove('anim-done');animateArchiveStat(node);}
  function mountArchiveWordStat(root){if(!root||!root.querySelector('[data-daybook-word-stat-pending]'))return;var lang=root.getAttribute('data-daybook-archive-language')||'zh-CN';if(Object.prototype.hasOwnProperty.call(archiveWordTotals,lang)){applyArchiveWordTotal(lang,archiveWordTotals[lang],root);return;}var endpoint=window.__htmlServerLiveData;if(!endpoint||root.hasAttribute('data-daybook-word-stat-loading'))return;root.setAttribute('data-daybook-word-stat-loading','');fetch(endpoint+'?kind=daybook-archive-summary&lang='+encodeURIComponent(lang),{headers:{Accept:'application/json'}}).then(function(response){if(!response.ok)throw new Error('archive summary '+response.status);return response.json();}).then(function(payload){var total=Number(payload&&payload.words);if(Number.isFinite(total))applyArchiveWordTotal(lang,total,currentArchiveRoot(lang));}).catch(function(){}).finally(function(){var current=currentArchiveRoot(lang);if(current)current.removeAttribute('data-daybook-word-stat-loading');});}
  function applyArchiveSiteStats(stats){if(!stats)return;var visitors=Number(stats.visitors)||0,visits=Number(stats.visits==null?stats.views:stats.visits)||0,visitorNode=q('[data-site-visitors-anim]'),viewNode=q('[data-site-views-anim]'),viewerNode=q('[data-site-viewers]'),visitorLabel=q('[data-site-visitors-label]'),viewLabel=q('[data-site-views-label]');if(visitorNode)visitorNode.setAttribute('data-target',String(visitors));if(viewNode)viewNode.setAttribute('data-target',String(visits));if(visitorLabel)visitorLabel.textContent=visitors===1?'visitor':'visitors';if(viewLabel)viewLabel.textContent=visits===1?'k view':'k views';if(viewerNode&&Number.isFinite(Number(stats.viewers))){viewerNode.textContent=String(Number(stats.viewers));viewerNode.classList.add('anim-done');}document.dispatchEvent(new CustomEvent('daybook:stats-loaded'));}
  function applyArchivePresence(value){var viewers=Number(value&&value.siteViewers),node=q('[data-site-viewers]');if(!node||!Number.isFinite(viewers))return;node.setAttribute('data-target',String(viewers));node.classList.remove('anim-done');document.dispatchEvent(new CustomEvent('daybook:stats-loaded'));}
  document.addEventListener('daybook:stats-loaded',mountArchiveStats);document.addEventListener('html-server:blog-site-stats',function(event){applyArchiveSiteStats(event.detail&&event.detail.stats);});document.addEventListener('html-server:blog-presence',function(event){applyArchivePresence(event.detail);});
  var mountedArchivePages=new WeakSet();
  function mountArchivePageEffects(event){var scope=event&&event.detail&&event.detail.main,root=scope&&scope.matches&&scope.matches('[data-daybook-archive-language]')?scope:scope&&scope.querySelector?scope.querySelector('[data-daybook-archive-language]'):null;if(!root){var roots=qa('[data-daybook-archive-language]');root=roots[roots.length-1];}if(!root||mountedArchivePages.has(root))return;mountedArchivePages.add(root);archiveSeenRows.clear();mountArchiveReveal(root);mountArchiveStats(root);mountArchiveWordStat(root);}
  mountArchivePageEffects();document.addEventListener('html-server:after-page-mount',mountArchivePageEffects);document.addEventListener('daybook:page-load',mountArchivePageEffects);
  function mountGraph(){var canvas=q('[data-daybook-graph-canvas]');if(!canvas||canvas.dataset.graphMounted==='true')return;canvas.dataset.graphMounted='true';var shell=canvas.closest('.html-server-daybook-graph-shell'),stage=canvas.querySelector('[data-daybook-graph-stage]'),nodes=qa('[data-graph-node]',canvas),links=qa('.html-server-daybook-graph-link',canvas),scale=1,panX=0,panY=0,drag=null;function transform(){stage.setAttribute('transform','translate('+panX+' '+panY+') scale('+scale+')');}function connected(id){var result={};result[id]=true;links.forEach(function(line){var source=line.getAttribute('data-source'),target=line.getAttribute('data-target');if(source===id)result[target]=true;if(target===id)result[source]=true;});return result;}canvas.addEventListener('pointerover',function(event){var node=event.target.closest&&event.target.closest('[data-graph-node]');if(!node)return;var active=connected(node.getAttribute('data-graph-node'));canvas.classList.add('graph-dimmed');nodes.forEach(function(value){value.classList.toggle('is-highlight',!!active[value.getAttribute('data-graph-node')]);value.classList.toggle('is-hovered',value===node);});links.forEach(function(line){line.classList.toggle('is-highlight',line.getAttribute('data-source')===node.getAttribute('data-graph-node')||line.getAttribute('data-target')===node.getAttribute('data-graph-node'));});});canvas.addEventListener('pointerout',function(event){if(event.relatedTarget&&event.relatedTarget.closest&&event.relatedTarget.closest('[data-graph-node]'))return;canvas.classList.remove('graph-dimmed');nodes.forEach(function(value){value.classList.remove('is-highlight','is-hovered');});links.forEach(function(line){line.classList.remove('is-highlight');});});canvas.addEventListener('wheel',function(event){event.preventDefault();scale=Math.max(.2,Math.min(5,scale*(event.deltaY<0?1.12:.89)));transform();},{passive:false});canvas.addEventListener('pointerdown',function(event){if(event.target.closest&&event.target.closest('[data-graph-node]'))return;drag={x:event.clientX,y:event.clientY,panX:panX,panY:panY};canvas.setPointerCapture(event.pointerId);});canvas.addEventListener('pointermove',function(event){if(!drag)return;panX=drag.panX+(event.clientX-drag.x)/scale;panY=drag.panY+(event.clientY-drag.y)/scale;transform();});canvas.addEventListener('pointerup',function(){drag=null;});var searchButton=q('[data-daybook-graph-search]'),input=q('[data-daybook-graph-search-input]'),tagsButton=q('[data-daybook-graph-tags]'),orphanButton=q('[data-daybook-graph-orphans]'),attachmentButton=q('[data-daybook-graph-attachments]'),resetButton=q('[data-daybook-graph-reset]');if(searchButton)searchButton.addEventListener('click',function(){var open=!shell.classList.contains('is-search-open');shell.classList.toggle('is-search-open',open);searchButton.setAttribute('aria-expanded',String(open));if(open&&input)input.focus();});if(input)input.addEventListener('input',function(){var term=input.value.trim().toLocaleLowerCase(),matches=0;nodes.forEach(function(node){var match=!term||(node.getAttribute('data-graph-title')||'').includes(term);node.classList.toggle('is-search-match',!!term&&match);node.classList.toggle('is-search-hidden',!!term&&!match);if(match)matches++;});canvas.classList.toggle('is-searching',!!term);});if(tagsButton)tagsButton.addEventListener('click',function(){var open=!shell.classList.contains('is-tags-visible');shell.classList.toggle('is-tags-visible',open);tagsButton.setAttribute('aria-pressed',String(open));});if(attachmentButton)attachmentButton.addEventListener('click',function(){var open=!shell.classList.contains('is-attachments-visible');shell.classList.toggle('is-attachments-visible',open);attachmentButton.setAttribute('aria-pressed',String(open));});if(orphanButton)orphanButton.addEventListener('click',function(){var show=orphanButton.getAttribute('aria-pressed')!=='true';orphanButton.setAttribute('aria-pressed',String(show));var connectedIds={};links.forEach(function(line){connectedIds[line.getAttribute('data-source')]=true;connectedIds[line.getAttribute('data-target')]=true;});nodes.forEach(function(node){if(!node.hasAttribute('data-graph-tag')&&!node.hasAttribute('data-graph-attachment'))node.classList.toggle('is-orphan-hidden',!show&&!connectedIds[node.getAttribute('data-graph-node')]);});});if(resetButton)resetButton.addEventListener('click',function(){scale=1;panX=0;panY=0;transform();});}
  mountGraph();document.addEventListener('html-server:after-page-mount',mountGraph);
  var install=function(api){api.registerLayoutHooks('daybook',{handlePageClick:function(event,services){var link=event.target.closest&&event.target.closest('a[href]');if(!link||event.defaultPrevented||event.button!==0||event.metaKey||event.ctrlKey||event.shiftKey||event.altKey||link.target==='_blank'||link.hasAttribute('download'))return;drawer(false);var target=new URL(link.href,location.href);if(target.origin!==location.origin)return;var cleanPath=function(value){return value.replace(/index[.]html$/,'').replace(/\/+$/,'')||'/';},homePath=cleanPath(new URL(services.siteUrl(''),location.href).pathname),currentHome=document.body.dataset.blogPageKind==='home',targetHome=cleanPath(target.pathname)===homePath,ownsNavigation=!!link.closest('.html-server-daybook-side-rail,.html-server-daybook-nav,.html-server-daybook-drawer,.html-server-daybook-mobile-bar');if(currentHome!==targetHome){event.preventDefault();event.stopImmediatePropagation();services.hardNavigate(target.href,false);return;}if(ownsNavigation&&(target.pathname!==location.pathname||target.search!==location.search)){event.preventDefault();event.stopImmediatePropagation();services.loadBlogPage(target.href,true);}}})};var api=window.HtmlServerBlog;if(api&&typeof api.registerLayoutHooks==='function')install(api);else(window.__htmlServerBlogContributions=window.__htmlServerBlogContributions||[]).push(install);
})();
(async function(){
